...
`tools/list` at `https://gateway.pipeworx.io/timeapi-io/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/current_time \
  -H 'Content-Type: application/json' \
  -d '{"timeZone":"America/New_York"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/current_time`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "timeapi-io": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-timeapi-io"]
    }
  }
}
```

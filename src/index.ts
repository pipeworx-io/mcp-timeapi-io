...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'TimeAPI.io');
}
...
const tools: McpToolExport['tools'] = [
  { name: 'current_time', description: 'Return the current date and time for a given IANA timezone (e.g. America/New_York): year, month, day, hour, minute, second, millisecond, dayOfWeek, dstActive.', inputSchema: { type: 'object', properties: { timeZone: { type: 'string' } }, required: ['timeZone'] } },
  {
  ...
  { name: 'current_iso_zoned_date_time', description: 'Return the current date/time for a given IANA timezone as a full ISO 8601 zoned datetime string (e.g. 2026-06-24T10:30:00+02:00).', inputSchema: { type: 'object', properties: { timeZone: { type: 'string' } }, required: ['timeZone'] } },
  { name: 'current_zone_info', description: 'Return timezone metadata for an IANA zone name: UTC offset, DST offset, current offset, DST start/end, and whether DST is currently active.', inputSchema: { type: 'object', properties: { timeZone: { type: 'string' } }, required: ['timeZone'] } },
  {
  ...
  { name: 'list_zones', description: 'Return all IANA timezone names supported by TimeAPI.io. Use as a directory to find valid timeZone strings for other calls.', inputSchema: { type: 'object', properties: {} } },
  {
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  const get = async (path: string, params?: Record<string, unknown>) => {
    ...
    if (!res.ok) throw await httpError(res, 'TimeAPI');
    return res.json();
  };
